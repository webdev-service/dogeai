...
async function setLocale(newLocale) {
  ...
  const linkImg = "/images/flags/" + newLocale + ".svg";

  $("#translation-icon").attr("src", linkImg);
  translatePage();
}
...
async function fetchTranslationsFor(newLocale) {
  const response = await fetch(`/lang/${newLocale}.json?ver=${Date.now()}`);
...
const links = document.querySelectorAll('.dropdown-list span');
...
links.forEach((element) => {
  element.addEventListener('click', function(evt) {
    // console.log(element.getAttribute("data-la"))
    span.innerHTML = evt.currentTarget.textContent;
    setLocale(element.getAttribute("data-la"));
  })
})
